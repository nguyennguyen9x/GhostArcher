...
export class UIManager {
    ...
    public showDialog (panelPath: string, args?: any, cb?: Function, panelPriority: number = Constant.PRIORITY.NORMAL) {
        ...
        ResourceUtil.createUI(panelPath).then((node: Node) => {
            ...
            // node.getComponent(UITransform).priority = panelPriority;
     ...
     * @memberof uiManager
     */
    public showMonsterBloodBar (scriptParent: any, totalBlood: number, hpAddition: number, callback: Function = () => { }, offsetPos: Vec3 = v3_monsterBloodOffsetPos) {
    ...
    public showBloodTips (scriptParent: any, type: number, bloodNum: number, offset: Vec3, callback?: Function) {
        ResourceUtil.getUIPrefabRes('common/fightTip').then((prefab: any) => {
            let ndTip = <Node>PoolManager.instance.getNode(prefab, <Node>find("Canvas"));
            let cameraCom = find("Main Camera")?.getComponent(CameraComponent) as CameraComponent;
            let pos = cameraCom?.convertToUINode(scriptParent.node.worldPosition, find('Canvas') as Node) as Vec3;
            pos.add(offset);
